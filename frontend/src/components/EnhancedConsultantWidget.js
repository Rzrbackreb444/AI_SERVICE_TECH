...
const EnhancedConsultantWidget = () => {
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {showNotification && !isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 right-1 bg-gradient-to-r from-emerald-600 to-cyan-600 text-white px-6 py-4 rounded-2xl shadow-2xl max-w-sm cursor-pointer border-2 border-emerald-400/50"
            style={{ zIndex: 9998 }}
            onClick={() => {
              setIsOpen(true);
      ...
      {/* SURGICAL WATERMARK PRECISION STRIKE */}
      <div className="fixed bottom-1 right-1" style={{ zIndex: 10000 }}>
        <AnimatePresence>
          {isOpen && (
            <motion.div
