...
RATE_LIMITS = {
    'free': {'requests_per_hour': 5, 'analyses_per_day': 1},
    'analyzer': {'requests_per_hour': 50, 'analyses_per_day': 10},
    'intelligence': {'requests_per_hour': 100, 'analyses_per_day': 25},
    'optimization': {'requests_per_hour': 200, 'analyses_per_day': 50},
    'portfolio': {'requests_per_hour': 500, 'analyses_per_day': 100},
    'watch_pro': {'requests_per_hour': 1000, 'analyses_per_day': 250}
}

async def check_rate_limit(user_id: str, tier: str, endpoint: str) -> bool:
    """Check if user has exceeded rate limits"""
    try:
        limits = RATE_LIMITS.get(tier, RATE_LIMITS['free'])
        now = datetime.now(timezone.utc)
        hour_key = f"rate_limit:{user_id}:{endpoint}:{now.strftime('%Y-%m-%d-%H')}"
